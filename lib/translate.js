                ...
                if (!error && response.statusCode == 200) {
                    ...
                    if(data.error_code){
                        ...
                        trans_result.forEach((item,index)=>{
                            map[item.src] = String(item.dst).toUpperCase()
                            result[keyList[index]] = String(item.dst).toUpperCase()
                        })
                    }
                    setTimeout(()=>{
                        resolve();
...
module.exports = async function () {
    ...
    if (fs.existsSync(config_file)) {
        ...
        console.log(`${config_file}文件不存在，请先初始化项目`.red);
        return;
    }
    if (fs.existsSync(config.dir+config.file)) {
        ...
        console.log(`需要翻译的文件不存在,${config.dir+config.file}`.red);
        return;
    }
    // 已有翻译的数据
    let oldData = {}
    ...
    const fromDataArray = Object.keys(fromData).filter(val=>{ return oldDataArray.indexOf(val) === -1 })

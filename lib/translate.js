...
// gpt翻译
const gptTranslate = async (val, chatGPTData) => {
    ...
    try {
        const response = await openai.createChatCompletion({
            ...
            messages: [
                ...
                {"role": "user","content": `I want you to act as an ${toCode} translator. 
                Your translated content will be applied on the user interface of the software.
                Keep the meaning the same, but be as concise as possible. 
                return only content, the return content not allow trailing commas. 
                My first content is: ${val}`}
            ],
        })
        let choice = response.data.choices[0].message
        let trans_result = []
        trans_result = choice.content.split('<GPT>\n') //翻译后的数组
        Object.keys(chatGPTData).forEach((key, index) => {
            const src = chatGPTData[key] || '' //原文
            let dst = trans_result[index] || '' // 译文
            if (toCode === 'en') {
            ...
            if (translateState) {
                if (mode === 'chatGPT') { //chatGPT翻译
                    if (String(query).length >= 600) {
                        translateState = await gptTranslate(query, chatGPTData);
                        query = fromData[key]
                        chatGPTData = {
                            key: query
                        }

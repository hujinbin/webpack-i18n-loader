...
// 用 new Function 动态加载文件，完全绕过 require 缓存
...
module.exports = async function (code, filename) {
    // 所有状态变量声明在函数作用域内，避免多次调用时的状态污染
    let map = {} // 遍历映射中英文对象
    let result = {}; // map 中英文对照表，result 翻译结果
    let fromData = {}; // 需要翻译的中文包
    let config = {} // 项目配置文件
    let toCode = '' //要翻译的语言
    let OPENAI_API_KEY = '' // AI key
    let AI_MODEL = '' // AI模型
    let AI_BASE_URL = '' // AI服务地址
    ...
    // 百度翻译（内部函数，捕获上方局部变量）
    const baiduTranslate = (val, keyList) => {
    ...
    OPENAI_API_KEY = config.aiKey || config.openAiKey // AI翻译密钥，兼容旧配置 openAiKey
    AI_MODEL = config.aiModel || '' // AI模型名称，不填默认 gpt-3.5-turbo
    AI_BASE_URL = config.aiBaseUrl || '' // AI服务地址，不填默认 OpenAI 官方地址
    for (let i = 0; i < distLangsLength; i++) {
        ...
        for (let j = 0; j < totalTranslateNum; j++) {
            const key = fromDataArray[j]
            if (translateState) {
                if (mode !== 'Baidu') { // AI翻译（兼容 OpenAI 及所有 OpenAI 兼容接口）
                    ...
                    if (String(query).length >= singleNum) {
                        translateState = await baiduTranslate(query, keyList);
                        query = fromData[key]
                        keyList = [key]
                        if (j === totalTranslateNum - 1) {
                            translateState = await baiduTranslate(query, keyList);
        ...
        if (translateState || translateNum > 0) { // translateState 翻译成功写入文件 || translateNum > 0 ：分段翻译出现部分失败
            ...
            const outFile = path.join(process.cwd(), filePath + '.js');
            fs.writeFileSync(outFile, `module.exports = ${JSON.stringify(result, null, '\t')};`, 'utf8');

  ...
  content = content.replace(/(['"`])(((?!\1).)*[\u4e00-\u9fa5]+((?!\1).)*)\1/gm, (value) => {
    ...
    if(prevIndex>-1 && nextIndex>-1){
      ...
      const cleanStr = value.replace(/\s*/g,"");
       // 类型2
      // return <span style="color: #52C718">已激活</span>
      // 这种类型的特殊处理 <span style="color: #52C718">已激活</span>
      if(nextIndex < prevIndex && cleanStr.indexOf('</')>-1){
        endIndex = value.indexOf('</')
      }
      const index = prevIndex+1
      prev = value.substr(0,index);
      next = value.substr(endIndex,value.length-1);
      value = value.substr(index,endIndex-index);
      value = `"${value}"`
    }

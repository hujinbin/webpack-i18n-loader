...
exports.generateTemplate = (messages, content, needReplace) => {
  ...
  let ignoreIndex = 0;
  // 替换忽略的部分
  content = content.replace(/<!--\s*<i18n-ignore(.|\s)*i18n-ignore>\s*-->/gim, (match, p1, p2, p3, offset, str) => {
    //排除掉url协议部分
...
exports.generateScript = (messages, content, needReplace) => {
  ...
  let ignoreIndex = 0;
  // 替换忽略的部分
  content = content.replace(/\s*<i18n-ignore(.|\s)*i18n-ignore>/gim, (match, p1, p2, p3, offset, str) => {
    //排除掉url协议部分
    if (match.indexOf("/*ignore_") > -1) return match;
    if (offset > 0 && str[offset - 1] === ':') return match;
    let ignoresKey = `/*ignore_${ignoreIndex++}*/`;
